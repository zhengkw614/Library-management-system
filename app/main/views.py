...
from .forms import LoginForm,RegisterForm,AddBookForm,EditBookForm,SearchBookForm,\
    SearchUserForm,AdminUserForm,AdminPasswdForm,SysSetForm,AddVoteForm
from app import db
from app.models import User,Book,Library,Request,SysInfo,Want,Statics,category,choices,Log, Vote, VoteChoice, VoteRecord
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and user.verify_password(form.password.data):
            ...
            if form.validcode.data.lower() != session.get('validcode'):
            # if False:
                flash('验证码错误')
            else:
                # 清除验证码缓存
                try:
                    ...
                    pass
                log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 登录了系统', date=datetime.now())
                db.session.add(log)
                db.session.commit()
                if user.user_type == 0:
...
@login_required
def logout():
    log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 退出了系统', date=datetime.now())
    db.session.add(log)
    db.session.commit()
    logout_user()
    flash('您已退出')
...
def sysinfo():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("修改成功")
        log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 修改了系统设置', date=datetime.now())
        db.session.add(log)
        db.session.commit()
...
def adminuser():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("修改成功")
        log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 修改了个人信息', date=datetime.now())
        db.session.add(log)
        db.session.commit()
...
def adminpasswd():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash('修改成功')
            log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 修改了密码', date=datetime.now())
            db.session.add(log)
            db.session.commit()
...
# 地理天气
@main.route('/geoweather/geoweather',methods=['GET','POST'])
@login_required
def geoweather():
    return render_template('geoweather.html')

@main.route('/geoweather/geoweatherframe',methods=['GET','POST'])
@login_required
def geoweatherframe():
    return render_template('geoweatherframe.html')

# --------------------------------- 天气api --------------------------------#
@main.route('/api/weather',methods=['GET','POST'])
@login_required
def weather_api():
    import requests
    url = "https://devapi.qweather.com/v7/weather/3d?key=e535cf0229d64262ac09a4c1c2503ba4&location=116.41,39.92"  #换成你自己的key
        ...
        "days": [
            {
                ...
                "day": res["daily"][0]["fxDate"],
                "sky": res["daily"][0]["textDay"],
                "wind": res["daily"][0]["windDirDay"],
                "icon": res["daily"][0]["iconDay"],
            },
            {
                "tempmin": res["daily"][1]["tempMin"],
                "tempmax": res["daily"][1]["tempMax"],
                "day": res["daily"][1]["fxDate"],
                "sky": res["daily"][1]["textDay"],
                "wind": res["daily"][1]["windDirDay"],
                "icon": res["daily"][1]["iconDay"]
            },
            {
                "tempmin": res["daily"][2]["tempMin"],
                "tempmax": res["daily"][2]["tempMax"],
                "day": res["daily"][2]["fxDate"],
                "sky": res["daily"][2]["textDay"],
                "wind": res["daily"][2]["windDirDay"],
                "icon": res["daily"][2]["iconDay"]
            }
...
# 设计问卷
@main.route('/vote/setvote',methods=['GET','POST'])
@login_required
def setvote():
    form = AddVoteForm()
    if form.validate_on_submit():
        ...
        for vote in votes:
            ...
            db.session.commit()
        votechoices = VoteChoice.query.all()
        for choice in votechoices:
            db.session.delete(choice)
            db.session.commit()
        voterecords = VoteRecord.query.all()
        ...
        if len(choices) < total_choice:
            ...
            for choice in choices:
                ...
                i = i + 1
            flash('发布成功')
        return redirect(url_for('main.setvote'))
    return render_template('setvote.html', form=form)


# 查看问卷结果
@main.route('/vote/voteresult',methods=['GET','POST'])
@login_required
def voteresult():
    vote_tmp = Vote.query.first()
    vote = {}
    if vote_tmp:
        vote["title"] = vote_tmp.title
        vote["content"] = vote_tmp.content
        vote["max_choice"] = vote_tmp.max_choice
        vote["start_time"] = vote_tmp.start_time.strftime('%Y-%m-%d %H:%M:%S')
        vote["end_time"] = vote_tmp.end_time.strftime('%Y-%m-%d %H:%M:%S')
    choices_tmp = VoteChoice.query.filter_by(vote_id=vote_tmp.id).all()
    choices = []
    i = 1
    for item in choices_tmp:
        records = VoteRecord.query.filter_by(result=item.code).all()
        choices.append({
            "no": i,
            "choice": item.choice,
            "count": len(records)
        ...
        i = i + 1
    return render_template('voteresult.html', vote=vote, choices=choices)




# 日志系统
@main.route('/log/syslog',methods=['GET','POST'])
@login_required
def syslog():
    logs_tmp = Log.query.all()
    logs = []
    if logs_tmp:
        for item in logs_tmp:
            logs.append({
                "date": item.date.strftime("%Y-%m-%d"),
                "content": item.content
            })
    return render_template('syslog.html', logs=logs)
...
def addinventory_api():
    ...
    if book:
        ...
        db.session.commit()
        result = {"status":0}
        log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 增加了一本书', date=datetime.now())
        db.session.add(log)
        db.session.commit()
    else:
        result = {"status":1}
...
def bookdelete_api():
    ...
    if book and blen == 0:
        ...
        db.session.commit()
        result = {"status":0}
        log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 删除了一本书', date=datetime.now())
        db.session.add(log)
        db.session.commit()
    else:
        result = {"status":1}
    ...
    if info['option']=='1':
        ...
        for book in books:
            ...
            data_row = {"sno": i, "isbn": book.isbn, "name": book.name, "author": book.author,
                        ...
                        "total": total, "free": free}
            data.append(data_row)
    log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 搜索了一本书', date=datetime.now())
    db.session.add(log)
    db.session.commit()
    result = {
        "code": 0,
