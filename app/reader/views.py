...
from app import db
from app.models import User,Book,Library,Request,SysInfo,Want,Statics,category,choices, Log, VoteChoice,Vote,VoteRecord
...
# 地理天气
@reader.route('/geoweather/geoweather',methods=['GET','POST'])
@login_required
def geoweather():
    return render_template('reader/geoweather.html')

@reader.route('/geoweather/geoweatherframe',methods=['GET','POST'])
@login_required
def geoweatherframe():
    return render_template('reader/geoweatherframe.html')

# --------------------------------- 天气api --------------------------------#
@reader.route('/api/weather',methods=['GET','POST'])
@login_required
def weather_api():
    import requests
...
# 参加投票
@reader.route('/user/dovote', methods=['GET','POST'])
@login_required
def dovote():
    if request.method == "GET":
        ...
        return render_template('reader/dovote.html', title=vote.title, content=vote.content,
        ...
        if vote_records:
            flash('您已参加过此投票，不能再投')
            return redirect(url_for('reader.dovote',title=vote.title))
...
def bookborrow_api():
    ...
    if count > 0:
        ...
        db.session.commit()
        result = {"status":0}
        log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 借阅了一本书', date=datetime.now())
        db.session.add(log)
        db.session.commit()
    else:
        result = {"status":2}
    ...
    for req in reqs:
        if req.opcode == 0:
            ...
            row_data = {"sno": i,
                        ...
                        "author": book.author,
                        "press": book.press,
                        "requestdate": datetime.fromtimestamp(req.requestdate).strftime("%Y-%m-%d %H:%M:%S")}
...
def withdraw_api():
    ...
    db.session.commit()
    result = {"status": 0}
    log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 撤回了一次借阅', date=datetime.now())
    db.session.add(log)
    db.session.commit()
    return jsonify(result)
...
def returnok_api():
    ...
    db.session.commit()
    result = {"status":0}
    log = Log(uid=current_user.id, content='用户 ' + current_user.username + ' 归还了一本书', date=datetime.now())
    db.session.add(log)
    db.session.commit()
    return jsonify(result)
